...
Displays side-by-side:
  ...
  Right: Search image (10 nm/px) with predicted bounding box and GT marker
"""
...
def visualize_prediction(model_path, search_path, ref_path, gt_x=None, gt_y=None):
    ...
    with torch.no_grad():
        with torch.amp.autocast('cuda'):
            heatmap = model(ref_tensor, search_tensor)  # [1, 1, 125, 125]

    # Extract prediction using argmax on flattened heatmap
    ...
    # Ground truth marker (GREEN circle) if provided
    ...
    title_str = f"Search (10 nm/px) | Pred: ({pred_x:.1f}, {pred_y:.1f})"
    if err is not None:
        title_str += f" | Err: {err:.1f}px"
    axes[1].set_title(title_str, fontsize=14, pad=10)
    axes[1].imshow(cv2.cvtColor(search_cv2, cv2.COLOR_BGR2RGB))
    axes[1].axis('off')

    axes[2].set_title("Predicted Heatmap", fontsize=14, pad=10)
...
if __name__ == "__main__":
    ...
    # Load the first sample's ground truth for visual comparison
